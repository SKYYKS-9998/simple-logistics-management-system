...
private:
	...
	void loadPacketUserInfo(streampos loc, PackUserInfo& recver) const;	//加载收件人信息，参数loc是收件人的信息在用户信息文件的位置
	...
	streampos searchID() const;					//返回当前对象的用户信息在文件中位置
	streampos searchID(const string& id) const;	//返回用户ID为id的用户信息在文件中的位置
	void writeToFile(int mode) const;			//将用户信息写入文件，mode=0写入新用户信息，mode=1更改当前用户信息
...
private:
	...
	streampos searchID() const;					//返回当前快递员的信息在快递员信息文件中的位置
